...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    ...
    import("socket.io-client").then(({ default: socketio }) => {
      io = socketio;
      const s = io(SOCKET_URL, { transports: ["websocket"] });
      setSocket(s);

      s.on("stockPrice", (data: any) => {
        setStockData(data);
        if (data.price && data.time) {
          setPriceHistory((prev) => {
            if (prev.length > 0 && prev[prev.length - 1].time === data.time)
              return prev;
            return [...prev.slice(-49), { time: data.time, price: data.price }];
          });
        }
      });

      s.on("stockCandles", (data: any) => {
        setCandleData(data);
      });

      s.emit("getStockPrice", symbol);
      s.emit("getStockCandles", symbol);
    });
  ...
  const handleSymbolChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setCandleData(null);
    if (socket) {
      socket.emit("getStockPrice", newSym);
      socket.emit("getStockCandles", newSym);
    }
  };
